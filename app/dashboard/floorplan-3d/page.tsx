...
export default function Floorplan3DPage() {
  ...
  useEffect(() => {
    ...
    const loadProjects = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await fetchProjects("floorplan-3d");
        setProjects(data);
      } catch (e) {
        console.error(e);
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-white">Floorplan to 3D Projects</h1>
        <p className="text-white/60 mt-2">Manage your floorplan conversions.</p>
      </div>

      {/* Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {/* Create New Card */}
        <NewProjectCard href="/dashboard/floorplan-3d/new" />
      </div>
